...
namespace Minesweeper_WPF
{
    ...
    class CounterControl : Grid
    {
        private const int NumberWidth = 13;

        private const int NumberHeight = 23;

        public static readonly BitmapSource NumbersImageSource;
        public static readonly DependencyProperty NumberProperty;

        static CounterControl()
        {
            NumbersImageSource = new BitmapImage(ImagesUri.NumbersUri);
        ...
        private static void NumberChangedCallback(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs args)
        {
        ...
        private static Image GetCroppedBitmap(NumberType numberType)
        {
            ...
            var bitmap = new CroppedBitmap(NumbersImageSource, new Int32Rect(0, offset * NumberHeight, NumberWidth, NumberHeight));

            return new Image { Source = bitmap };
        }

        private void GetLastDigit(ref int number, out int result)
        {
            result = 0;
            if (number == 0)
            {
